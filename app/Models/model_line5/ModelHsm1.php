...
class ModelHsm1 extends Model
{
    ...
    {
        return $this->select('L5_HSM1_TYPE_BATTERY,L5_HSM1_TEMP_LEFT, L5_HSM1_TEMP_SET_VALUE,waktu')
            ->orderBy('waktu', 'DESC')
            ->limit(50)
            ->find();
    }
    ...
    {
        return $this->select('L5_HSM1_TYPE_BATTERY,L5_HSM1_TEMP_RIGHT, L5_HSM1_TEMP_SET_VALUE,waktu')
            ->orderBy('waktu', 'DESC')
            ->limit(50)
            ->find();
    }
    ...
    public function getDistinctTempLeftbyDate($date)
    {
        ...
        return $this->select("CASE 
                            WHEN L5_HSM1_TEMP_LEFT < 360 THEN 'TEMP<360' 
                            WHEN L5_HSM1_TEMP_LEFT BETWEEN 360 AND 369.9 THEN '360-369.9' 
                            WHEN L5_HSM1_TEMP_LEFT BETWEEN 370 AND 379.9 THEN '370-379.9' 
                            WHEN L5_HSM1_TEMP_LEFT BETWEEN 380 AND 389.9 THEN '380-389.9' 
                            WHEN L5_HSM1_TEMP_LEFT BETWEEN 390 AND 399.9 THEN '390-399.9' 
                            ...
                            WHEN L5_HSM1_TEMP_LEFT BETWEEN 450 AND 459.9 THEN '450-459.9' 
                            WHEN L5_HSM1_TEMP_LEFT BETWEEN 460 AND 470.9 THEN '460-470.9' 
                            WHEN L5_HSM1_TEMP_LEFT > 470.9 THEN 'TEMP>471' 
                        END AS TEMP_LEFT_RANGE, 
                        COUNT(*) AS count")
        ->where('waktu >=', $startTime)
            ->where('waktu <', $endTime)
    ...
    public function getDistinctTempRightbyDate($date)
    {
        ...
        return $this->select("CASE 
                            WHEN L5_HSM1_TEMP_RIGHT < 360 THEN 'TEMP<360'   
                            WHEN L5_HSM1_TEMP_RIGHT BETWEEN 360 AND 369.9 THEN '360-369.9' 
                            WHEN L5_HSM1_TEMP_RIGHT BETWEEN 370 AND 379.9 THEN '370-379.9' 
                            WHEN L5_HSM1_TEMP_RIGHT BETWEEN 380 AND 389.9 THEN '380-389.9' 
                            WHEN L5_HSM1_TEMP_RIGHT BETWEEN 390 AND 399.9 THEN '390-399.9' 
                            ...
                            WHEN L5_HSM1_TEMP_RIGHT BETWEEN 450 AND 459.9 THEN '450-459.9' 
                            WHEN L5_HSM1_TEMP_RIGHT BETWEEN 460 AND 470.9 THEN '460-470.9' 
                            WHEN L5_HSM1_TEMP_RIGHT > 470.9 THEN 'Temp>471'  
                        END AS TEMP_RIGHT_RANGE, 
                        COUNT(*) AS count")
        ->where('waktu >=', $startTime)
            ->where('waktu <', $endTime)
    ...
    public function getDataTempLeftByDate($date)
    {
        ...
        foreach ($data as $row) {
            ...
            $tempLeft = $row['L5_HSM1_TEMP_LEFT']; // Pastikan Anda mengambil L5_HSM1_TEMP_LEFT yang benar
    ...
    public function getLastBatteryTypeTime()
    {
       ...
       WITH BatteryChanges AS (
            SELECT 
                CASE 
                    WHEN L5_HSM1_TYPE_BATTERY IN ('0', '2') THEN '0'  -- Treat '0' and '2' as the same
                    ELSE L5_HSM1_TYPE_BATTERY 
                END AS normalized_battery_type,
                waktu, 
                ROW_NUMBER() OVER (PARTITION BY CASE 
                                                WHEN L5_HSM1_TYPE_BATTERY IN ('0', '2') THEN '0' 
                                                ELSE L5_HSM1_TYPE_BATTERY 
                                             END 
            ...
            WHERE L5_HSM1_TYPE_BATTERY <> previous_battery_type OR previous_battery_type IS NULL
        ) 
        SELECT 
            waktu, 
            normalized_battery_type AS L5_HSM1_TYPE_BATTERY 
        FROM BatteryChanges     
        WHERE rn = 1;
    ");
    ...
    public function getDataGreen()
    {
        ...
        foreach ($shifts as $key => $shift) {
            ...
            $count = $this->where("CONVERT(DATE, waktu) = ", $today) // Mengambil bagian tanggal dari kolom waktu
                ->where('L5_HSM1_TEMP_LEFT <', '435')
                ->where('L5_HSM1_TEMP_LEFT >', '415')
                ->where('L5_HSM1_TEMP_RIGHT <', '435')
                ->where('L5_HSM1_TEMP_RIGHT >', '415')
    ...
    public function getDataTempLeftByWeek($date1, $date2)
    {
        ...
        foreach ($data as $row) {
            $batteryType = $row['L5_HSM1_TYPE_BATTERY'];
            $tempLeft = $row['L5_HSM1_TEMP_LEFT'];
