...
<script>
    ...
    const fetchDataByWeek = (selectedDate, selectedDate2) => {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (!data.data || data.data.length === 0) {
                    ...
                    console.log('No data received or data is empty');
                    updateOrCreateChartMeltingweek(
                        'lidholder', chart_lidholder, 'Lid Holder Position',
                        [], [], 'L5_HSM2_LID_HOLDER_ACTUAL_POS',
                        [], 'L5_HSM2_LID_HOLDER_MELTING_POS',
                        (newChart) => {
        ...
        //distinct week
        $.ajax({
            url: `<?= base_url('hsmline5/distinct/right2/week') ?>`,
            method: 'POST',
            data: fd,
            processData: false,
            contentType: false,
            ...
            error: (jqXHR, textStatus, errorThrown) => {
                console.error('Error fetching data for temp right hsm 2 distinct:', textStatus, errorThrown);
            }
        });
        $.ajax({
            url: `<?= base_url('hsmline5/distinct/left2/week') ?>`,
            method: 'POST',
            data: fd,
            processData: false,
            contentType: false,
    ...
    const fetchData_tempLeft = () => {
        $.ajax({
            ...
            dataType: 'json',
            success: (data) => {
                createChart('temp_left', '<?= base_url('hsmline5/distinct/left2') ?>', 'TEMP_LEFT_RANGE', 'Temperatur Left');
            },
            error: (jqXHR, textStatus, errorThrown) => {
    ...
    const fetchData_tempRight = () => {
        $.ajax({
            ...
            dataType: 'json',
            success: (data) => {
                createChart('temp_right', '<?= base_url('hsmline5/distinct/right2') ?>', 'TEMP_RIGHT_RANGE', 'Temperatur Right');
            },
            error: (jqXHR, textStatus, errorThrown) => {
