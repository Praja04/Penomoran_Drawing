...
<script>
            ...
            success: (data) => {
                if (!data.data || data.data.length === 0) {
                    ...
                    updateOrCreateChart2(
                        [], [], 'container', chart, 'APB2 - TEMP RIGHT Actual',
                        [], [], 'L6_APB2_TEMP_RIGHT_SETTING',
                        [], 'L6_APB2_TEMP_RIGHT_ACTUAL',
                        (newChart) => {
                            chart = newChart;
                        },
                        [], []
        ...
        $.ajax({
            ...
            success: (data) => {
                if (!data.data || data.data.length === 0) {
                    ...
                    console.log('No data received or data is empty');
                    updateOrCreateChart2(
                        [], [], 'container2', chart3, 'APB2 - TEMP LEFT Actual',
                        [], [], 'L6_APB2_TEMP_LEFT_SETTING',
                        [], 'L6_APB2_TEMP_LEFT_ACTUAL',
                        (newChart) => {
                            chart3 = newChart;
                        },
                        [], []
    ...
    const fetchDataByWeek = (selectedDate, selectedDate2) => {
        ...
        //distinct
        $.ajax({
            url: `<?= base_url('apbline6/distinct/right2/week') ?>`,
            method: 'POST',
            data: fd,
            processData: false,
            contentType: false,
            ...
            error: (jqXHR, textStatus, errorThrown) => {
                console.error('Error fetching data for temp right apb 2 distinct:', textStatus, errorThrown);
            }
        });
        $.ajax({
            url: `<?= base_url('apbline6/distinct/left2/week') ?>`,
            method: 'POST',
            data: fd,
            processData: false,
            contentType: false,
    ...
    $('#data-source').on('change', function() {
        ...
        if (selectedSource === 'date') {
            ...
            // Reset charts
            chart = chart3 = chart_tempLeft = chart_tempRight = null;
